...
export default function GoogleSignInButton({ isLoading, onSuccess, onError }) {
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      let errorMessage = 'Google inloggen mislukt. Probeer opnieuw.';
      
      if (error.code === 'auth/popup-closed-by-user') {
        errorMessage = 'Popup gesloten. Probeer opnieuw.';
      } else if (error.code === 'auth/cancelled-popup-request') {
        errorMessage = 'Login geannuleerd.';
      } else if (error.message) {
        errorMessage = `Google inloggen mislukt: ${error.message}`;
      }
